  ...
  const handlePaste = useCallback((e) => {
    ...
    if (/^[0-9]{4}$/.test(text)) {
      ...
      inputsRef.current.forEach(
        (input, index) => (input.value = newOtp[index])
      );
      ...
      <div className="max-w-md mx-auto text-center bg-white px-4 sm:px-8 py-10 rounded-xl shadow">
        <header className="mb-8">
          <h1 className="text-2xl font-bold mb-1">
            Vérification de téléphone mobile
          </h1>
          <p className="text-[15px] text-slate-500">
            Entrez le code de vérification à 4 chiffres envoyé à votre numéro de
            téléphone : <strong>{phoneNumber}</strong>. Est-il correct ?
          </p>
          <p className="text-[15px] text-slate-500">
